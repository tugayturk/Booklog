...
export const LibraryProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const addBookToLibrary = async (bookId: string) => {
        if (!userId) return;
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/book/${userId}/library`, { bookId });
    ...
    const removeBookFromLibrary = async (bookId: string) => {
        if (!userId) return;
        const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/book/${userId}/library`, { data: { bookId } });
        await getUserLibrary();
        return response.data;
    }
